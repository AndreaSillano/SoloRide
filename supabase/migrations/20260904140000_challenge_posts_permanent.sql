-- Challenge posts are permanent (kept in the ride). Allow a challenge share
-- after today's cadence post without a second schedule-satisfying permanent.

-- Cadence uniqueness: one non-challenge permanent post per ride/user/date.
drop index if exists public.posts_ride_user_date_permanent_key;

create unique index posts_ride_user_date_permanent_key
  on public.posts (ride_id, user_id, scheduled_date)
  where is_temporary = false and ride_challenge_id is null;

comment on index public.posts_ride_user_date_permanent_key is
  'One schedule cadence permanent post per ride/user/day. Challenge completions are separate permanent rows.';
